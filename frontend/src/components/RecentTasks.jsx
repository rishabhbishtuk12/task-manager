      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e5eaf2] px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[#667085]">
            Latest work across the project board.
          </p>
        </div>
        <button
          className="rounded-lg border border-[#dce3ee] bg-white px-4 py-2 text-sm font-semibold text-slate-600 cursor-pointer"
          type="button"
...
function StatusPill({ status }) {
  ...
  return (
    <span
      className={`rounded-lg px-3 py-1.5 text-sm font-medium ${styles[status]}`}
    >
      {formatStatus(status)}
    </span>
  );
